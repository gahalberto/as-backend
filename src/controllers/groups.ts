...
export const addGroup:RequestHandler = async (req, res) => {
    ...
    if(!newGroup) return res.json({error: "Ocorreu algum erro"});
...
export const deleteGroup:RequestHandler = async (req, res) => {
    ...
    const deletedGroup = groups.remove({
        id: parseInt(id),
        id_event: parseInt(id_event)
    });

    if(deletedGroup) return res.status(201).json({group: deletedGroup});
    res.json({error: "Aconteceu algum erro"});
}